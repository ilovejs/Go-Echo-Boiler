...
//			return err
//		}
//		//Association append mode => article +tags
...
//		return nil, 0, err
//	}
//	as.db.Model(&t).Preload("Favorites").Preload("Tags").
//		Preload("Author").Offset(offset).Limit(limit).Order("created_at desc").
...
//		return nil, 0, err
//	}
//	as.db.Where(&model.Article{AuthorID: u.ID}).Preload("Favorites").Preload("Tags").Preload("Author").
...
//	}
//
//	as.db.Model(&u).Preload("Favorites").Preload("Tags").Preload("Author").Offset(offset).Limit(limit).
//		Order("created_at desc").Association("Favorites").Find(&articles)
//	//trick
...
//	as.db.Model(&u).Preload("Following").Preload("Follower").Association("Followings").
//		Find(&followings)
//
//	var ids []uint
...
//	}
//	as.db.Where("author_id in (?)", ids).Preload("Favorites").
//		Preload("Tags").Preload("Author").Offset(offset).Limit(limit).Order("created_at desc").
//		Find(&articles)
//
//	as.db.Where(&model.Article{AuthorID: u.ID}).Model(&model.Article{}).Count(&count)
//	return articles, count, nil
...
//	var m model.Article
//	/* Pulling relevant object via 'Preload in 3 select:
//
//	SELECT * FROM "articles"  WHERE "articles"."deleted_at" IS NULL AND (("articles"."slug" = 'how-to-train-your-dragon'))
//	SELECT * FROM "comments"  WHERE "comments"."deleted_at" IS NULL AND (("article_id" IN (1))) ORDER BY "comments"."id" ASC
//	SELECT * FROM "users"  WHERE "users"."deleted_at" IS NULL AND (("id" IN (1))) ORDER BY "users"."id" ASC
//	*/
//	if err := as.db.Where(&model.Article{Slug: slug}).
//		Preload("Comments").
